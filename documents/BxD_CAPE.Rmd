...
scale.pheno <- TRUE
remove.outlier <- TRUE
high.var.pheno.only = FALSE #whether to include only high-variance traits in the analysis
include.genetic.markers = FALSE
```
...
all.packages <- c("cape", "pheatmap", "qtl2", "knitr", "abind")
load_libraries(all.packages)
```

```{r read_data}
#read in phenotype data
data.set <- read_cross2(here("data", "qtl2_data", "BxD.yaml"))
...
```{r high_var}
trait.var <- apply(pheno, 2, var)
var.thresh = summary(trait.var)[2]
hist(trait.var, breaks = 100)
abline(v = var.thresh, col = "red")
high.var <- which(trait.var > var.thresh)
if(high.var.pheno.only){
  var.pheno <- pheno[,high.var]
}else{
  var.pheno <- pheno
}
```
...
```{r scale_pheno}
if(scale.pheno){
  scaled.pheno <- apply(var.pheno, 2, scale)
  rownames(scaled.pheno) <- rownames(var.pheno)
}else{
